...
with open(course_info) as new_file, open("results.txt", "w") as results, open("results.csv", "w") as result:
    data = []
    for row in new_file:
        parts = row.split(":")
        data.append(parts[1].strip())
    results.write(f"{data[0]}, {data[1]} credits\n")
    results.write("======================================\n")
    ...
    for id, name in names.items():
        ...
        if exam_total + (exercise_points) < 15:
            ...
            grade = 5
        result.write(f"{id};{name};{grade}\n")
        results.write(f"{name:30}{exercise_total:<10}{exercise_points:<10}{exam_total:<10}{exam_total+exercise_points:<10}{grade:<10}\n")
